...
def _build_model(max_len, num_features, target_chars, target_chars_group, model_arch:NN_model, resource, encoder):
    ...
    if encoder == Encodings.One_hot:
        ...
        processed_act = embedding_act
        processed_res = embedding_res
    else:
        main_input = Input(shape=(num_features,), name='main_input')
        if resource:
            embedding = Embedding(input_dim=target_chars * target_chars_group if encoder==Encodings.Shrunk_Index_based else target_chars + target_chars_group
    ...
    if model_arch == NN_model.LSTM:
        ...
        if encoder == Encodings.One_hot:
            ...
            transformer = TransformerEncoder(intermediate_dim=64, num_heads=4)
        ...
        if resource:
            if encoder == Encodings.Multi_encoders:
                transformer = TransformerEncoder(intermediate_dim=64, num_heads=4)
                processed = transformer(res_modulator)
                processed = GlobalMaxPooling1D()(processed)
...
def _train_model(model, checkpoint_name, x, y_a, y_g,encoder):
    ...
    if (y_g is None) :
        history = model.fit(x, y_a, validation_split=shared.validation_split,
...
def train(log_data: LogData, model_arch, encoder:Encodings,output_folder:Path, X, y_a, y_g=None):
    ...
    checkpoint_name = create_checkpoints_path(log_data.log_name,model_type, output_folder)
