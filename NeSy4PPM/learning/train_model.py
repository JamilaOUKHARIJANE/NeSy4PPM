...
from keras.layers import LSTM, Dense, Input, Dropout, BatchNormalization, GlobalMaxPooling1D, Embedding, Concatenate
...
from NeSy4PPM.commons.utils import Encodings, NN_model
from NeSy4PPM.learning.Modulator import Modulator
from NeSy4PPM.learning.prefixes_preprocessing import extract_encode_prefixes
from NeSy4PPM.learning.train_common import create_checkpoints_path, plot_loss
import absl.logging
...
def train(log_data: LogData, encoder:Encodings, model_arch: NN_model, x, y_a, y_g=None, output_folder:Path=shared.output_folder):
    ...
    for fold in range(shared.folds):
        model = _build_model(maxlen, x.shape[-1], y_a.shape[1], y_g.shape[1] if y_g is not None else 0, model_arch, resource,encoder)
        checkpoint_name = create_checkpoints_path(log_data.log_name, model_arch, fold, model_type,encoder,output_folder)
...
def learn(log_data: LogData, encoder:Encodings, model_arch: NN_model, resource: bool,output_folder:Path=shared.output_folder):
    ...
    train(log_data, encoder, model_arch, output_folder, x, y_a, y_g)