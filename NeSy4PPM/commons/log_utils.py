...
class LogData:
    ...
    def __init__(self, log_path:Path,log_name=None,train_ratio=0.8, train_log=None, test_log=None, case_name_key = 'case:concept:name',act_name_key = 'concept:name'
                 ,res_name_key = 'org:resource',timestamp_key = 'time:timestamp',resource = False):
        ...
        if self.log_name is not None and self.train_log_name is None and self.test_log_name is None:
            self.log= self.read_log(log_path, log_name,resource)
            ...
            train_log = self.read_log(log_path, self.train_log_name,resource)
            test_log = self.read_log(log_path, self.test_log_name,resource)
    ...
    def read_log(self, log_path, log_name,resource):
